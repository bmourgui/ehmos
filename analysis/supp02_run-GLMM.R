...
foreach (s=1:2) %:%
  ...
  data_jags <- list(n = dim(Z)[2], J = dim(Z)[1], S = dim(Z)[3],
                    ...
                    alti = sc.site[, s],
                    survey = c(0, 1))
  name <- paste("out.sim_asym_A", s,"_glmm", r, sep="")
...
for (s in 1:2){
  for (r in 1:30){
    name <- here::here("results", paste0("out.sim_asym_", dimnames(Z)[[4]][s],"_glmm", r, ".RData"))
    load(name)
    res.glmm_asym[[v]] <- out
    names(res.glmm_asym)[v] <- paste0("out.sim_asym_", dimnames(Z)[[4]][s],"_glmm", r)
    v <- v+1
  }
}

save(res.glmm_asym, file=here::here("results", "simu_asym_out-glmm.RData"))
